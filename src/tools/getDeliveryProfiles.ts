...
type GetDeliveryProfilesInput = z.infer<typeof GetDeliveryProfilesInputSchema>;
...
const getDeliveryProfiles = {
  ...
  execute: async (args: GetDeliveryProfilesInput) => {
    try {
      const query = gql`
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { deliveryProfiles: response.deliveryProfiles.nodes },
              null,
              2,
            ),
          },
