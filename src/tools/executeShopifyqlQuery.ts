...
const ShopifyqlQueryInputSchema = z.object({
  query: z
    .string()
    .describe(
      "The ShopifyQL query string. Must include a FROM and SHOW clause. " +
...
const executeShopifyqlQuery = {
  ...
  description:
    "Execute custom analytics and reporting queries using ShopifyQL (requires read_reports scope). Use to fetch aggregated sales, order, or customer metrics.",
  schema: ShopifyqlQueryInputSchema,
  initialize(client: GraphQLClient) {
    shopifyClient = client;
  },
  execute: async (args: ShopifyqlQueryInput) => {
    try {
      const query = gql`
      ...
      if (result.parseErrors && result.parseErrors.length > 0) {
        throw new Error(
          `ShopifyQL Parse Errors: ${result.parseErrors
            .map((e: any) => e.message)
            .join(", ")}`,
        );
      }
            ...
            text: JSON.stringify(
              {
                ...
                rows:
                  result.tableData?.unformattedData ||
