...
const EditOrderInputSchema = z.object({
    ...
    .array(
      z.object({
        variantId: z
          .string()
          .describe(
            "The ID of the product variant to add (gid://shopify/ProductVariant/...)",
          ),
    ...
    .array(
      z.object({
        lineItemId: z
          .string()
          .describe(
            "The ID of the EXISTING line item to adjust (gid://shopify/LineItem/...)",
          ),
        quantity: z
          .number()
          .describe(
            "The NEW total quantity for this line item (0 to completely remove)",
          ),
        restock: z
          .boolean()
          .default(true)
  ...
  notifyCustomer: z
    ...
    .default(false)
    .describe(
      "Whether to send an email notification to the customer about the edit",
    ),
  staffNote: z
    .string()
    .optional()
...
type EditOrderInput = z.infer<typeof EditOrderInputSchema>;
...
const editOrder = {
  name: "order-edit",
  description:
    "Begin, modify, and commit edits to an existing order (add variants, custom items, or modify/remove quantities).",
  schema: EditOrderInputSchema,
  initialize(client: GraphQLClient) {
    shopifyClient = client;
  },
  execute: async (args: EditOrderInput) => {
    try {
      ...
      `;
      const beginRes = await shopifyClient.request<any>(beginMutation, {
        id: args.orderId,
      });
      ...
      if (args.addVariants && args.addVariants.length > 0) {
        for (const variant of args.addVariants) {
          ...
          `;
          const res = await shopifyClient.request<any>(addVarMutation, {
            id: calculatedOrderId,
            variantId: variant.variantId,
            quantity: variant.quantity,
          });
          checkUserErrors(
            res.orderEditAddVariant.userErrors,
            "execute mutation",
          );
        }
      }
      ...
      if (args.addCustomItems && args.addCustomItems.length > 0) {
        for (const item of args.addCustomItems) {
          ...
          `;
          const res = await shopifyClient.request<any>(addCustomMutation, {
            id: calculatedOrderId,
            title: item.title,
          ...
          checkUserErrors(
            res.orderEditAddCustomItem.userErrors,
            "execute mutation",
          );
        }
      }
      ...
      if (args.setQuantities && args.setQuantities.length > 0) {
        for (const item of args.setQuantities) {
          ...
          const calcLineItemId = item.lineItemId.replace(
            "LineItem",
            "CalculatedLineItem",
          );
          ...
          `;
          const res = await shopifyClient.request<any>(setQtyMutation, {
            id: calculatedOrderId,
          ...
          checkUserErrors(
            res.orderEditSetQuantity.userErrors,
            "execute mutation",
          );
        }
      }
      ...
      `;
      const commitRes = await shopifyClient.request<any>(commitMutation, {
        id: calculatedOrderId,
        ...
        content: [
          {
            type: "text",
            text: JSON.stringify(
              { order: commitRes.orderEditCommit.order },
              null,
              2,
            ),
