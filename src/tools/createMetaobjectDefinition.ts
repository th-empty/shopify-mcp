...
const MetaobjectFieldDefinitionCreateInputSchema = z.object({
  ...
  type: z
    .string()
    .describe(
      "e.g. 'single_line_text_field', 'multi_line_text_field', 'number_integer'",
    ),
...
const CreateMetaobjectDefinitionInputSchema = z.object({
  type: z
    .string()
    .describe(
      "The type namespace. Prefix with '$app:' to reserve for your app's exclusive use",
    ),
  name: z.string(),
  description: z.string().optional(),
  fieldDefinitions: z
...
type CreateMetaobjectDefinitionInput = z.infer<
  typeof CreateMetaobjectDefinitionInputSchema
>;
...
export const createMetaobjectDefinition = {
  ...
  description:
    "Creates a metaobject definition that establishes the structure for custom data objects",
  ...
  execute: async (input: CreateMetaobjectDefinitionInput) => {
    try {
      ...
      return {
        success: true,
        metaobjectDefinition:
          response.metaobjectDefinitionCreate?.metaobjectDefinition,
      };
    } catch (error) {
